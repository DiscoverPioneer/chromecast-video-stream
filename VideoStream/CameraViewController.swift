...
class CameraViewController: UIViewController {
    ...
    let pgc = PioneerGoogleCast()
    // MARK: - Lifecycle

    @objc func startStream() {
        print("Preparing to stream to: \(ipAddress)")
        pgc.cast(ipAddress)
    }
    ...
    override func viewDidLoad() {
        ...
        let stream = UIBarButtonItem(title: "Stream", style: .plain, target: self, action: #selector(CameraViewController.startStream))
        navigationItem.leftBarButtonItem = stream
        ...
        print("Creating IP \(String(describing: self.ip))")
        ...
        print("JUST SET NEW IP: \(ipAddress)")
